...
<ul>
  ...
  <li><details>
  <summary>How can I track use of datasets I've published?</summary>

  OBIS logs every download and exposes two API endpoints for accessing the counts. Both are read-only and require no authentication.

  **Yearly download counts**

  Use `/metrics` to see per-year totals for a dataset:

  `https://api.obis.org/v3/metrics?datasetid=<dataset-uuid>`
  ...
  {
    "downloads": [
      { "year": 2026, "records": 5586, "downloads": 138 },
      { "year": 2025, "records": 12034, "downloads": 412 }
    ]
  }
  ```

  `records` is the total number of occurrence records served across all downloads that year. `downloads` is the number of distinct download events.

  You can also query at the OBIS node level with `nodeid=<node-uuid>` to see aggregate use across every dataset published by a node.

  **Downloads within a specific time window**

  For quarterly reports, annual reviews, or any window that doesn't align to calendar years, use `/metrics/downloads`:

  `https://api.obis.org/v3/metrics/downloads?datasetid=<dataset-uuid>&startdate=2026-01-01&enddate=2026-03-31`

  Response:

  ```json
  { "records": 5586, "downloads": 138 }
  ```

  Both `startdate` and `enddate` are optional; leave them off to get totals across all history.

  To see every individual download event with its timestamp, add `groupby=time`:

  `https://api.obis.org/v3/metrics/downloads?datasetid=<dataset-uuid>&startdate=2026-01-01&enddate=2026-03-31&groupby=time`

  This returns one entry per download, which is useful for spotting usage patterns — for example, how often the full dataset is being downloaded versus filtered subsets.

  **Finding your dataset UUID**

  Your dataset's UUID appears in the URL of its OBIS dataset page (`https://obis.org/dataset/<uuid>`), and in the `id` field of `https://api.obis.org/v3/dataset?scientificname=<name>` search results.
